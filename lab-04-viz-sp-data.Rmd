...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for on denney's restaurant. The variables are: `r names(dennys)`.***

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?

***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for on La Quinta restaurant. The variables are: `r names(laquinta)`.***
...
***La Quinta hotels are mostly located in the Americas but also have locations in other parts of world. Denney's are located in the US.***
...
***We can use the state variable to determine whether a location is in the US. Specifically by comparing the listed to a database of states in the US.***
...
laquinta %>%
  mutate(country = case_when(
    ...
    state == "ANT"           ~ "Colombia",
    state == "AG"            ~ "Maldves"     # fill in the rest
  ))
```
